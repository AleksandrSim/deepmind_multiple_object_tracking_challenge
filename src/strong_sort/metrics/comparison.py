...
def run_iou(boxes: Dict[str, Any], db_dict: Dict[str, Any], filter=True, moving= False, stationary=False) -> Dict[str, Any]:
...
#          continue
      ...
      if stationary and 'moving' in gt_track and gt_track['moving'] == False:
        pred_bb = np.stack([pred_bb[:, 0]*width, pred_bb[:, 1]*height,pred_w*width, pred_h*height], axis=1)
         ...
         pred_bb = np.stack([pred_bb[:, 0], pred_bb[:, 1],pred_w, pred_h], axis=1)
