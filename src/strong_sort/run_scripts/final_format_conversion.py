...
sys.path.append('src/strong_sort/run_scripts')
from interpolation import GSI
...
class ConvertFinalFormat:
    ...
    def iterate_videos(self):
        ...
        for seq in sequences:
            ...
            video_name = seq
            print(f'generating file format {video_name}')
            original = np.genfromtxt(os.path.join(curr_dir, 'tracker.txt'), delimiter=',', dtype=float)
            if self.GSI:
                print('inside GSI')
                interpolater = GSI(data=os.path.join(curr_dir, 'tracker.txt'))
                interpolater.save_to_text_file(path=os.path.join(curr_dir, 'tracker_extrapolated.txt'))
                original = np.genfromtxt(os.path.join(curr_dir, 'tracker_extrapolated.txt'), delimiter=',', dtype=float)
