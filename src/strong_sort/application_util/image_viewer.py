...
class ImageViewer(object):
    ...
    * 'ESC' : quit
    ...
    def rectangle(self, x, y, w, h, label=None, gt=False):
        """Draw a rectangle.
        ...
        if label is not None and not gt:
        ...
        elif label is not None and gt:
            thick_rect = 5  # adjust the thickness as you like
            cv2.rectangle(self.image, pt1, pt2, (0, 0, 0), thick_rect)
            
            # Display a larger text "This is GT" near the rectangle
            gt_label_pt = int(x + w + 5), int(y + h // 2)
            cv2.putText(self.image, "This is GT", gt_label_pt, cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
            # If label is provided, add it inside the rectangle
            text_size = cv2.getTextSize(str(label) + "_GT", cv2.FONT_HERSHEY_PLAIN, 2, thick_rect)
            center = pt1[0] + 5, pt1[1] + 5 + text_size[0][1]
            cv2.putText(self.image, str(label) + "_GT", center, cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), thick_rect)
    ...
    def run(self, update_fun=None, dir_to_save=None):
        if update_fun is not None:
            self._user_fun = update_fun

        if dir_to_save is not None:
            video_filename = os.path.basename(dir_to_save).replace('sequence_dir', '') + '.mp4'
            full_save_path = os.path.join(dir_to_save, video_filename)
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            self._video_writer = cv2.VideoWriter(full_save_path, fourcc, 30, (640, 480))
        ...
        while not self._terminate:
            ...
            if not is_paused:
                ...
                if self._video_writer is not None:
                    self._video_writer.write(cv2.resize(self.image, (640, 480)))
            t1 = time.time()
            remaining_time = max(1, int(self._update_ms - 1e3 * (t1 - t0)))
            if dir_to_save is None:
                cv2.imshow(self._caption, cv2.resize(self.image, (640, 480)))
